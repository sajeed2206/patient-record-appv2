...
def add_patient():
    ...
    if request.method == 'POST':
        ...
        # Append to dataframe using concat instead of deprecated append
...
def edit_patient(patient_id):
    ...
    patients = pd.read_csv('data/patients.csv')
    patient_rows = patients[patients['id'] == patient_id]
    
    if patient_rows.empty:
        flash('Patient not found', 'danger')
        return redirect(url_for('dashboard'))
        
    patient = patient_rows.to_dict('records')[0]
...
def export_csv():
    ...
    return send_file(
        ...
        mimetype='text/csv',
        as_attachment=True,
        download_name='patients_export.csv'  # Updated from attachment_filename
    )
...
def export_template():
    """Export an empty CSV template with the required columns"""
    if 'username' not in session:
        return redirect(url_for('login'))
    
    # Create an empty dataframe with required columns
    template = pd.DataFrame(columns=['name', 'age', 'gender', 'contact', 'address', 'blood_group', 'conditions', 'medications'])
    ...
    return send_file(
        ...
        mimetype='text/csv',
        as_attachment=True,
        download_name='patient_template.csv'
    )
